document.addEventListener('DOMContentLoaded', function() {
  ...
  carousels.forEach(function( carousel ) {
      ...
      const bulletArr = Array.from(bullets);
      for (let i = slideArr.length - 1; i > 0; i--) {
          const j = Math.floor(Math.random() * (i + 1));
          [slideArr[i], slideArr[j]] = [slideArr[j], slideArr[i]];
          [bulletArr[i], bulletArr[j]] = [bulletArr[j], bulletArr[i]];
      }
      slideArr.forEach(function(slide) { ele.appendChild(slide); });
      bulletArr.forEach(function(bullet) { carousel.querySelector('ol').appendChild(bullet); });

      // Initialize the carousel
      nextarrow.style.display = 'block';
      prevarrow.style.display = 'block';
      ele.scrollLeft = 0;
      bulletArr[0].classList.add('selected');
      slideArr[0].classList.add('selected');
      if(amountvisible>1) {
      ...
      const updateHeight = function() {
          const currentSlide = carousel.querySelector('ul li.selected');
          if (currentSlide) {
      ...
      if (firstImg.complete) {
          ...
          firstImg.addEventListener('load', updateHeight);
      }
      window.addEventListener('resize', debounce(updateHeight));
